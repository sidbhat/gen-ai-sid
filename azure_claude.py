...
result_str = ""
response = ""
st.set_page_config(page_title="Anthropic Claude 2| SAP Generative AI| Sid Bhattacharya", page_icon=':rocket:')
c = st.container()
header = c.container()
...
header.header("🚀 Ask Chatty McChatface with Claude ")
header.caption(
    "Demo app that showcases how to do prompt engineering with Anthropic Claude and how to build an enterprise knowledge base using custom embeddings.")
bar = header.empty()
...
def download_docx(str_: str):
    ...
    doc.add_paragraph(f'Authored By: Chatty McChatface - Claude')
           ...
           with st.spinner("Fetching response..."):
              with output_container:
                    if st.session_state.conversations:
                        ...
                        ai_role = "You are a helpful SAP assistant that provides detailed answers based on facts. Always cite references for your responses towards the end of the response."  # NOQA: E501
                        st.session_state.conversations = [
                    ...
                    if st.session_state.bool_search:
                        ...
                        st.session_state.conversations.append(f"Human: {prompt_input} \\nAssistant:")
                        ...
                        st.session_state.conversations.append(f"{st.session_state.response}")
                        download_content = st.session_state.response
                        #print(conversation)
                        c.caption(":robot_face: Response from Anthropic Claude")
                    ...
                    if download_content != '':
                        c.download_button(
                                    ...
                                    file_name="Chatty McChatface Response Claude-" + datetime.datetime.now().strftime(
                                        "%m-%d-%Y-%H:%M:%S") + ".docx",
                                    mime="docx"
                                )
